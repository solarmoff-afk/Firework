...
fn test_analyze_conditional_widget() {
    ...
    assert!(
        !ir.screen_data
            .values()
            .any(|data| data.maybe_widgets.spark_widget_map.is_empty())
    );
}
...
fn test_analyze_widget_missing_key_error() {
    ...
    let (_, error, _) = prepare_tokens(syn::parse2(tokens).unwrap(), CompileFlags::new(), 0);
...
fn test_analyze_multiple_widget_types() {
    ...
    let widget_types: Vec<String> = ir
        .statements
        .iter()
        .filter_map(|s| {
