...
use crate::compiler::analyze::ClosureData;
...
pub enum FireworkAction {
    ...
    UpdateSpark(
        ...
        Option<String>,
        // Информация о том, находится ли стейтемент в замыкании. Reduce позволяет определить
        // является ли это замыканием без блока которое только меняет спарк, например:
        // let mut name = || spark += 1; Это замыкание нужно обработать иначе, гвард
        // здесь генерировать не нужно так как это замыкание, а не вызов здесь и сейчас
        // без этого все́ создание замыкания будет обе́рнуто в условие что создаст ошибку
        // при попытке использования. Также нужно делать из такого замыкания блок на
        // этапе кодогенерации. Если Some то он нкходится в замыкании (любом)
        Option<ClosureData>,
    ),
