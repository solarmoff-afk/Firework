...
impl<'a> CodegenVisitor<'a> {
    ...
    pub(crate) fn find_mask_counts(&mut self) {
        for (_screen_name, _screen_signature, screen_id) in self.ir.screens.iter() {
            ...
            let spark_count = match self.ir.screen_data.get(screen_id) {
                Some(data) => data.spark_count,
                None => 0,
            };
    ...
    pub(crate) fn find_widget_mask_counts(&mut self) {
        for (_screen_name, _screen_signature, screen_id) in self.ir.screens.iter() {
            ...
            let widget_count = match self.ir.screen_data.get(screen_id) {
                Some(data) => data.maybe_widgets.count,
                None => 0,
            };
