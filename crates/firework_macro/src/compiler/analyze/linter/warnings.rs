...
impl FireworkLinter {
    pub fn generate_cycle_warning(
        ...
        let parent_name = self.nodes_map.get(&id_parent)
            .map(|s| s.0.as_str())
            .unwrap_or("unknown");
        
        let child_name = self.nodes_map.get(&id_child)
            .map(|s| s.0.as_str())
            .unwrap_or("unknown");

        let child_code = self.nodes_map.get(&id_child)
            .map(|s| s.1.as_str())
            .unwrap_or("unknown");
        
        let flat_path = cycle_path.iter().map(|&id| {
            let name = self.nodes_map.get(&id)
                .map(|s| s.0.as_str())
                .unwrap_or("unknown");
        ...
        ascii_graph.push_str(&format!("             ╭──▶ {} (id {})\n", parent_name, id_parent));
        
        for &node_id in &unique_nodes[1..] {
            let node_name = self.nodes_map.get(&node_id)
                .map(|s| s.0.as_str())
                .unwrap_or("unknown");
