...
impl CodeGen {
    ...
    pub(crate) fn find_mask_counts(&mut self) {
        for (screen_name, _screen_signature, screen_id) in self.ir.screens.iter() {
    ...
    pub(crate) fn check_reactive_loop(
        ...
        if !old_reactive_loop_flag && statement.reactive_loop {
            ...
            *depth = "\t".repeat((depth_value + 1 + statement.depth).into());
            ...
            for mask_index in 0u8..mask_count {
                ...
                // локальная переменная которая будет дропнута
                screen_code.0.push_str(
                    format!("{}let _fwc_bitmask{}_clone = _fwc_bitmask{};\n",
                        depth, mask_index + 1, mask_index + 1).as_str());
        ...
        if old_reactive_loop_flag && !statement.reactive_loop {
            ...
            // При перезапуске кода нужно снять _fwc_event на Reactive чтобы не
            // перезапустить обработчики событий и другой код который зависит от
            // контекста экрана
            screen_code.0.push_str(
                format!("{}_fwc_event = firework_ui::LifeCycle::Reactive;\n", depth).as_str());
