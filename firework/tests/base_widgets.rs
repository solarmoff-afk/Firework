...
#[test]
fn test_ui_text() {
    let commands = TestHarness::run(test_ui_text_screen);

    assert_eq!(
        commands,
        vec![
            AdapterCommand::RemoveAll,
            AdapterCommand::NewText { layout: 1 },
            AdapterCommand::SetHitGroup(0, 65535),
            AdapterCommand::ClearText(0),
            AdapterCommand::PushText {
                handle: 0,
